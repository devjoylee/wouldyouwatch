...
export const movieAPI: Fetchers<MovieResponse> = {
  ...
  upcoming: () =>
    fetch(`${BASE_URL}/movie/now_playing?api_key=${API_KEY}&language=en-US&page=1&region=ca`).then(
      (res) => res.json()
    ),
  nowPlaying: () =>
...
export const tvAPI: Fetchers<TvResponse> = {
  ...
  airingToday: () =>
    fetch(`${BASE_URL}/tv/airing_today?api_key=${API_KEY}&language=en-US&page=1`).then((res) =>
      res.json()
    ),
  topRated: () =>
